...
export function renderPlatformSelectStep(props: ConfigWizardProps) {
  const { state, onLoadPlatforms } = props;

  // Load platforms on first render
    ...
    <div class="wizard-content">
      <div class="card">
        ...
        ${state.platformsLoading
          ? html`<div class="muted" style="margin-top: 24px;">Loading platforms...</div>`
...
function renderPlatformGrid(props: ConfigWizardProps) {
  ...
  const sortedPlatforms = [...state.platforms].sort((a, b) => {
  ...
  return html`
    <div class="platform-grid" style="margin-top: 24px; display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px;">
      ${sortedPlatforms.map(
        (platform) => renderPlatformCard(platform, state.selectedPlatformId, onSelectPlatform),
      )}
    </div>
  `;
}
...
function renderPlatformCard(
  platform: { id: string; label: string; description: string; configured: boolean; enabled: boolean },
  ...
  return html`
    ...
    >
      <div class="row" style="justify-content: space-between; align-items: start;">
        ...
        ${platform.enabled
          ? html`<span class="badge badge-success" style="font-size: 11px;">Active</span>`
          : platform.configured
